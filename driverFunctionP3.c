...
*
* File: driverFunctionP3.c
*
* Description: functions that include the copy implementation and moving files
*
**************************************************************/
...
void moveFunction( char *name, char *name1){
	...
	if(fileIndex==-1&&fileIndex1==-1){
		...
		if(strcmp(userInput2,userInput3)!=0){
			int subIndex1= subIndexOfFile(userInput2);
			dirEntries[cdLoc].subFiles[subIndex1]= strdup(" ");
			int subIndex2= subIndexOfFile(userInput3);
			dirEntries[cdLoc].subFiles[subIndex2]= strdup(userInput2);
			numOfFreeFiles+=1;
